...
export class EditRecipeService {
  ...
  getRecipe(id: string): Observable<ViewRecipe[]> {
    ...
    // updateRecipe(id: string): Promise<ViewRecipe[]> {
    //     const putURL = 'http://localhost:3000/recipe/' + id;
    //     return this.http.put(putURL, id)
    //     .toPromise()
    //     .then(response => response.json() as ViewRecipe[]);
